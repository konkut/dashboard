...
title="{{ __('lg.connect.twofactor') }} | D1oris APP"
>
    ...
    <div class="wrapper">
        <div class="css-page flex justify-center h-screen w-full text-center box-border">
            <div class="h-fit my-auto bg-white p-10 w-[480px] rounded-lg">
                ...
                <h2 class="text-[2c2c2c] block font-medium m-0 p-0 mt-4 text-center text-[1.5em]">{{__('lg.connect.twofactor')}}</h2>
        
                <div class="mt-4">
                    {!! Form::open(['url'=>'/','id'=>'form_connect_two_auth','autocomplete'=>'off'])!!}
                    {!!Form::hidden('autocomplete',null,['class'=>'autocomplete'])!!}
                    <label class="block mt-4 text-start" for="code">{{__('lg.connect.code')}}:</label>
                    ...
                    <p class="text-[0.9em] font-light text-[#444] leading-[1.14em] mt-4">{{__('lg.connect.code_text_1')}}</p>
                    <hr class="h-px bg-[#f0f0f0] border-0 my-0 p-0 mt-4">
                    <a href="{{ url('/connect/two/factor') }}" class="text-[#00b2ee] inline-block p-1 hover:text-[#0088b6] font-bold flex justify-center box-border m-0 p-0 mt-1">{{__('lg.connect.code_text_2')}}</a>
                </div>
            </div>
        </div>
    </div>
